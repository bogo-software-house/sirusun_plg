...
function AddOccupantModal({ onClose, onSuccess }) {
  ...
  const SelectButton = ({ label, isSelected, onClick, isDisabled }) => (
    <button
      className={`p-2 border rounded-md  ${isDisabled ? "bg-red-500 text-white cursor-not-allowed" : isSelected ? "bg-indigo-500 text-white" : "bg-gray-200 text-black"}`}
      onClick={onClick}
      disabled={isDisabled}
    >
      {label}
        ...
        {rooms.length > 0 && (
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-4 mt-2 p-4 border rounded-md bg-gray-50" style={{ maxHeight: "300px", overflowY: "auto" }}>
              ...
              ${
                isRoomAvailable
                  ? "bg-green-500 text-white" // kamar tersedia
                  : "bg-red-500 text-white"
              } // kamar terisi, beri warna merah
