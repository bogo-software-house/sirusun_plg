...
import { Roomcolumns } from "../../components/columns/RoomColumns";
...
function Bangunan() {
  ...
  const [ setConditionOptions] = useState([]); // Opsi kondisi default array kosong
  ...
  useEffect(() => {
    ...
    fetch(`https://api.sirusun.com/api/kamar/filter?${query.toString()}&page=${currentPage}`)
  ...
  const handleEditClick = (room) => {
    ...
    const initialValues = Object.entries(room.kondisi).reduce((acc, [key, value]) => {
  ...
  const confirmUpdate = async () => {
    try {
      ...
      const response = await fetch(`https://api.sirusun.com/api/rooms/${selectedRoom.custom_id}`, {
        method: "POST",
  ...
  return (
    <>
      <div>
        ...
        />
        <RoomFilterModal isOpen={isFilterModalOpen} onRequestClose={() => setIsFilterModalOpen(false)} filter={filter} handleFilterChange={handleFilterChange} />
        <Table
          showCheckbox={false}
          columns={Roomcolumns}
